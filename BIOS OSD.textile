...
You can use a PlayStation BIOS dump with POPS. Put a "BIOS.BIN" file in the VMC directory (example : __common/POPS/PBPX-95000.MY_GAME/BIOS.BIN) and POPStarter will use it for this game. You can also put it in the POPS folder, so it will be used for all games. If no BIOS are found in VMC directory or POPS folder, POPS will use its built-in BIOS.
Though using a BIOS with a game doesn't improve compatibility, you can use that feature to manage your MC, listen your CDDA games tracks...
