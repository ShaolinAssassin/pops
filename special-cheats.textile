...
| $NOPAL | Disables POPStarters' PAL patcher. | Can be done using "PATCH_9.BIN":https://bitbucket.org/ShaolinAssassin/popstarter-documentation-stuff/downloads/$NOPAL_PATCH.7z. |
| $FORCEPAL | Forces the activation of the PAL patcher (POPS will run it PAL) and patches the BIOS region code to Euro (shows the boot screen in PAL).  | Useful for PAL VCDs that don't have a valid license text in their bootsector. Can be done using "PATCH_8.BIN":https://bitbucket.org/ShaolinAssassin/popstarter-documentation-stuff/downloads/$FORCEPAL_PATCH.7z. |
