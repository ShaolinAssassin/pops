...
To use POPStarter in SMB mode, you have to find and download the compressed POPS file. This file is named "POPS_IOP.PAK" and it's the same as the USB mode uses... 
...
3. Copy POPS_IOP.PAK  into the "POPS" directory you've just created.
...
3. Rename the POPStarter ELF as the name of your VCDs, replace the .VCD extension with a .elf extension, add the SB. prefix;   [example : for running a VCD named "Gran Turismo.VCD", the POPStarter ELF must be renamed as "SB.Gran Turismo.elf"];
...
* You can NOT use the old POPS.ELF+IOPRP252.IMG files (POPS.PAK) in SMB mode, updating to POPS_IOP_PAK is mandatory for SMB mode.
...
h3. "*Home*":https://bitbucket.org/SaulGoodman/popstarter-documentation-stuff/wiki/Home